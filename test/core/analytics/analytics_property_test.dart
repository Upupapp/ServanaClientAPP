...
void main() {
  group('AmountBandValues.forAmount', () {
    test('0 → band0', () => expect(AmountBandValues.forAmount(0), AmountBandValues.band0));
    test('1 → band1to500', () => expect(AmountBandValues.forAmount(1), AmountBandValues.band1to500));
    test('500 → band1to500', () => expect(AmountBandValues.forAmount(500), AmountBandValues.band1to500));
    test('501 → band501to1000', () => expect(AmountBandValues.forAmount(501), AmountBandValues.band501to1000));
    test('1000 → band501to1000', () => expect(AmountBandValues.forAmount(1000), AmountBandValues.band501to1000));
    test('5001 → band5001plus', () => expect(AmountBandValues.forAmount(5001), AmountBandValues.band5001plus));
    test('negative → band0', () => expect(AmountBandValues.forAmount(-1), AmountBandValues.band0));
  });
  ...
  group('LatencyBucketValues.forMillis', () {
    test('0ms → <500ms', () => expect(LatencyBucketValues.forMillis(0), '<500ms'));
    test('499ms → <500ms', () => expect(LatencyBucketValues.forMillis(499), '<500ms'));
    test('500ms → 500-999ms', () => expect(LatencyBucketValues.forMillis(500), '500-999ms'));
    test('1000ms → 1-2s', () => expect(LatencyBucketValues.forMillis(1000), '1-2s'));
    test('5000ms → 5s+', () => expect(LatencyBucketValues.forMillis(5000), '5s+'));
    test('10000ms → 5s+', () => expect(LatencyBucketValues.forMillis(10000), '5s+'));
  });
