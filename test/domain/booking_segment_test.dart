...
void main() {
  group('BookingStatus enum', () {
    ...
    test('fromString maps Active statuses correctly', () {
      ...
      expect(
          BookingStatusMapper.fromString('STARTED'), BookingStatus.inProgress);
    ...
    test('fromString maps Completed statuses correctly', () {
      expect(
          BookingStatusMapper.fromString('COMPLETED'), BookingStatus.completed);
      expect(BookingStatusMapper.fromString('DONE'), BookingStatus.completed);
      expect(
          BookingStatusMapper.fromString('REVIEWED'), BookingStatus.reviewed);
    });

    test('fromString maps Cancelled statuses correctly', () {
      expect(
          BookingStatusMapper.fromString('CANCELLED'), BookingStatus.cancelled);
      expect(BookingStatusMapper.fromString('EXPIRED'), BookingStatus.expired);
      ...
      expect(
          BookingStatusMapper.fromString('REFUNDED'), BookingStatus.refunded);
    });

    test('fromString returns unknown for unrecognised values', () {
      expect(
          BookingStatusMapper.fromString('GIBBERISH'), BookingStatus.unknown);
    ...
    test('isGenuineSuccess is true for success terminal states', () {
      ...
      expect(BookingStatusMapper.isGenuineSuccess(BookingStatus.cancelled),
          isFalse);
      expect(
