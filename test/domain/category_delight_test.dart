...
void main() {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // CategoryRevealOverlay — widget smoke tests
  // ─────────────────────────────────────────────────────────────────────────

  group('CategoryRevealOverlay widget', () {
    setUp(() => CategoryExperienceHistory.resetForTesting());
    ...
    // Advance time past the maximum overlay lifetime (hold + exit) and drain
    // all pending timers so tests do not leak.
    Future<void> drainOverlay(WidgetTester tester) async {
      await tester.pump(const Duration(seconds: 2));
      await tester.pumpAndSettle();
    }

    testWidgets('Beauty first-view shows headline and subtext', (tester) async {
      const config = CategoryRegistry.beautyWellness;
      ...
      expect(find.text(config.revealHeadline), findsOneWidget);
      expect(find.text(config.revealSubtext), findsOneWidget);

      // Drain all timers before teardown.
      await drainOverlay(tester);
      expect(dismissed, isTrue);
    });

    testWidgets('Aircon overlay calls onDismiss after hold + exit animation',
        (tester) async {
      const config = CategoryRegistry.aircon;
      bool dismissed = false;
      ...
      // Advance past hold duration + exit fade.
      await tester.pump(
          config.firstViewRevealDuration + const Duration(milliseconds: 500));
      await tester.pumpAndSettle();
    ...
    testWidgets('Massage ripple rings do not cause overflow', (tester) async {
      const config = CategoryRegistry.massage;
    ...
    testWidgets('Generic overlay renders without error', (tester) async {
      const config = CategoryRegistry.generic;
