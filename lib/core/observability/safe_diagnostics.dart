...
abstract final class SafeDiagnostics {
  ...
  static String sanitizeMessage(Object error) {
    ...
    msg = msg.replaceAll(RegExp(r'(\+63|0)(9\d{9})'), '[phone_redacted]');
    // Remove URLs (could contain IDs or tokens)
    msg = msg.replaceAll(RegExp(r'https?://\S+'), '[url_redacted]');
    // Remove potential tokens (long alphanumeric strings > 30 chars)
    msg =
        msg.replaceAll(RegExp(r'\b[A-Za-z0-9_\-]{31,}\b'), '[token_redacted]');
  ...
  static String safeBreadcrumb(String template,
