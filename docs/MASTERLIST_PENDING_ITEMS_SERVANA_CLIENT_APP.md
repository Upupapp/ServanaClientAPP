...
| SC-179 | RELEASE | **freerasp 8.0.0 logs two permission errors at every launch.** `TalsecScreenProtector: Failed to register screenshot callback. Check if android.permission.DETECT_SCREEN_CAPTURE permission is granted` and the same for `DETECT_SCREEN_RECORDING`. Neither permission is declared in `android/app/src/main/AndroidManifest.xml` — observed on emulator-5554 at 18:45. freerasp 8 added screen-capture detection and expects these; without them that protection is silently inactive, which matters more once SC-166 is fixed and the threat signal is actually consumed. Adding them is a Play data-safety consideration, so it is a decision, not a mechanical fix. | client-mobile | yes | **verified — observed in logcat** |
